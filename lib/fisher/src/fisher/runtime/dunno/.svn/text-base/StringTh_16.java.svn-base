...
public  class  StringTh_16 extends ThingImmutable implements Applyable  { static String copyright() { return fisher.util.Copyright.IBM_COPYRIGHT; }
  

	public static final StringTh_16 EMPTY_STRING = new StringTh_16("");
	public String value;

	public static StringTh_16 of(String s) {
		if (s == null)
			throw new NullPointerException();
		return new StringTh_16(s);
	}

	private StringTh_16(String value) {
		super();
		this.value = value;
	}
	...
	public boolean equals(Object obj) {
		return obj != null && obj.getClass() == StringTh_16.class && ((StringTh_16) obj).value.equals(this.value);
	}
	...
	private StringTh_16 trim() {
		return StringTh_16.of(this.value.trim());
	}
	...
	private StringTh_16 CplusChar(String methodName, Thing[] args, Evaller evaller, Framelike frame, Syntax src)
	throws FisherException {
		checkNumberOfArgs(1, 1, methodName, args, evaller, frame, src);
		int i = args[0].asJavaInt(src);
		char c = (char) i;
		return StringTh_16.of(this.value + c);
	}
	...
	private StringTh_16 CtoUpper(String methodName, Thing[] args, Evaller evaller, Framelike frame, Syntax src)
			throws FisherException {
		checkNumberOfArgs(0, 0, methodName, args, evaller, frame, src);
		return StringTh_16.of(value.toUpperCase());
	}

	private StringTh_16 CtoLower(String methodName, Thing[] args, Evaller evaller, Framelike frame, Syntax src)
			throws FisherException {
		checkNumberOfArgs(0, 0, methodName, args, evaller, frame, src);
		return StringTh_16.of(value.toLowerCase());
	}

	private StringTh_16 Ccapitalize(String methodName, Thing[] args, Evaller evaller, Framelike frame, Syntax src)
			throws FisherException {
		checkNumberOfArgs(0, 0, methodName, args, evaller, frame, src);
		String c;
		if (value.length() == 0) {
			...
			c = value.substring(0, 1).toUpperCase() + value.substring(1).toLowerCase();
		}
		return StringTh_16.of(c);
	}

	private StringTh_16 Cplus(String methodName, Thing[] args, Evaller evaller, Framelike frame, Syntax src)
			throws FisherException {
		checkNumberOfArgs(1, 1, methodName, args, evaller, frame, src);
		String re = EvalUtil.toString(args[0]);
		return StringTh_16.of(value + re);
	}
	...
	public static Thing thornifyMatcher(Syntax src, MatchResult m) throws FisherException {
		...
		for (int g = 0; g <= m.groupCount(); g++) {
			...
			RecordTh rg = RecordTh.make(src, matcherFields, IntTh.of(start), IntTh.of(end), StringTh_16.of(group));
			L.add(rg);
		}
	...
	private StringTh_16 Mreplace(String methodName, Thing[] args, Evaller evaller, Framelike frame, Syntax src)
			throws FisherException {
		checkNumberOfArgs(2, 2, methodName, args, evaller, frame, src);
		String target = args[0].toString();
		String replacement = args[1].toString();
		return StringTh_16.of(this.value.replace(target, replacement));
	}

	private StringTh_16 Mreplace1(String methodName, Thing[] args, Evaller evaller, Framelike frame, Syntax src)
			throws FisherException {
		checkNumberOfArgs(2, 2, methodName, args, evaller, frame, src);
		String target = args[0].toString();
		String replacement = args[1].toString();
	...
	private StringTh_16 MreplaceRE(String methodName, Thing[] args, Evaller evaller, Framelike frame, Syntax src)
			throws FisherException {
		checkNumberOfArgs(2, 2, methodName, args, evaller, frame, src);
		String regexp = args[0].toString();
		String replacement = args[1].toString();
		return StringTh_16.of(this.value.replaceAll(regexp, replacement));
	}

	private StringTh_16 Mreplace1RE(String methodName, Thing[] args, Evaller evaller, Framelike frame, Syntax src)
			throws FisherException {
		checkNumberOfArgs(2, 2, methodName, args, evaller, frame, src);
		String regexp = args[0].toString();
		String replacement = args[1].toString();
		return StringTh_16.of(this.value.replaceFirst(regexp, replacement));
	}
	...
	private StringTh_16 Mspan(String methodName, Thing[] args, Evaller evaller, Framelike frame, Syntax src)
			throws FisherException {
		checkNumberOfArgs(2, 2, methodName, args, evaller, frame, src);
		...
		String ss = value.substring(start, end);
		return StringTh_16.of(ss);
	}
	...
	public Thing apply(Thing[] args, Framelike frame, Evaller evaller, Syntax src) throws FisherException {
		...
		if (args.length == 1) {
			...
			String sub;
			if (pi == len || pj <= pi) {
				return StringTh_16.EMPTY_STRING;
			} else if (pj < len) {
				sub = this.value.substring(pi, pj);
			} else {
				sub = this.value.substring(pi);
			}
			return StringTh_16.of(sub);
		} else {
